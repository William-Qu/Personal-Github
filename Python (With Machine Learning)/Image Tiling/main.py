...
def buildTriangles(slice0, slice1):
    ...
    # [1 mark]
    dist = 100000000
    closeVert0 = Vertex([0, 0, 0])
    closeVert1 = Vertex([0, 0, 0])
    closeVert0Index = 0
    closeVert1Index = 0
    for verts0 in range(len(slice0.verts)):
        for verts1 in range(len(slice1.verts)):

            distCheck = length(subtract(slice0.verts[verts0].coords, slice1.verts[verts1].coords))
            if distCheck <= dist:
                dist = distCheck
                closeVert0 = slice0.verts[verts0]
                closeVert1 = slice1.verts[verts1]
                closeVert0Index = verts0
                closeVert1Index = verts1
    ...
    if (slice0.verts[0].coords[0] != closeVert0.coords[0] or
            slice0.verts[0].coords[1] != closeVert0.coords[1] or
            slice0.verts[0].coords[2] != closeVert0.coords[2]):

        i = 0
        startNew0 = slice0.verts[closeVert0Index]
        cycle0.append(startNew0)

        while i < len(slice0.verts):
            startNew0 = startNew0.nextV
            cycle0.append(startNew0)
            i += 1
    else:
        i = 0
        sameStart = slice0.verts[0]
        cycle0.append(sameStart)

        while i < len(slice0.verts):
            sameStart  = sameStart.nextV
            cycle0.append(sameStart)
            i += 1

    #print("---")

    if (slice1.verts[0].coords[0] != closeVert1.coords[0] or
            slice1.verts[0].coords[1] != closeVert1.coords[1] or
            slice1.verts[0].coords[2] != closeVert1.coords[2]):

        i = 0
        startNew1 = slice1.verts[closeVert1Index]
        cycle1.append(startNew1)

        while i < len(slice1.verts):
            startNew1 = startNew1.nextV
            cycle1.append(startNew1)
            i += 1

    else:
        i = 0
        sameStart = slice1.verts[0]
        cycle1.append(sameStart)
    ...
    # [YOUR CODE HERE]
    numVerts0 = len(cycle0)
    numVerts1 = len(cycle1)
    ...
    for i in range(1, numVerts0):
        minArea[0][i] = triangleArea(cycle0[i - 1].coords, cycle1[0].coords, cycle0[i].coords) + minArea[0][i - 1]
    ...
    for i in range(1, numVerts1):
        minArea[i][0] = triangleArea(cycle1[i - 1].coords, cycle0[0].coords, cycle1[i].coords) + minArea[i - 1][0]
    ...
    for x in range(1, len(slice1.verts)+1):
        for y in range(1, len(slice0.verts)+1):
            leftMinimumArea = minArea[x][y - 1] + triangleArea(cycle0[y - 1].coords,
                                         cycle0[y].coords,
                                         cycle1[x].coords)
            upMinimumArea = minArea[x - 1][y] + triangleArea(cycle1[x - 1].coords,
                                       cycle0[y].coords,
                                       cycle1[x].coords)
            if (leftMinimumArea < upMinimumArea):
                minArea[x][y] = leftMinimumArea
                minDir[x][y] = Dir.PREV_COL
            else:
                minArea[x][y] = upMinimumArea
    ...
    while (numVerts1 != 0 and numVerts0 != 0):
        if minDir[numVerts1][numVerts0] == Dir.PREV_ROW:
            triangle_addition = Triangle([cycle1[numVerts1], cycle0[numVerts0], cycle1[numVerts1 - 1]])
            numVerts1 -= 1
            triangles.append(triangle_addition)
        else:
            triangle_addition = Triangle([cycle1[numVerts1], cycle0[numVerts0], cycle0[numVerts0 - 1]])
            numVerts0 -= 1
            triangles.append(triangle_addition)
