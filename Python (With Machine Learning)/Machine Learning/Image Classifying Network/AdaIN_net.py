...
class AdaIN_net(nn.Module):
    ...
    def forward(self, content, alpha=1.0):
        ...
        if self.training:  # training
            # Content loss is simply done with Relu4, but style loss is done with all the returned Relu's
            # VGG Encoder and AdaIn are static, Decoder is being trained to minimize loss
            # Input both images seperately to the VGG Encoder
            # Combine the encoded images using AdaIn, this will give you the combined encoded image
            # Decoder will give the combined image
            # Re-encode the combined image using VGG Encoder and use contentLoss() to get the content loss of the image
            # Encode the combined image and the style image and use styleLoss() to get the style loss of the image
            # Train the decoder
            # Then return loss of content and syle
            ...
            # Combine the encoded images using AdaIn, this will give you the combined encoded image
            # Could be style_features[-1]
            combined_features = self.adain(content_features, style_features)
            combined_features = alpha*combined_features + (1-alpha)*content_features

            # Decoder will give the combined image
            combined_image = self.decoder(combined_features)

            # Re-encode the combined image using VGG Encoder and use contentLoss() to get the content loss of the image
            combined_lossy_features = self.encode(combined_image)
            loss_c = self.content_loss(combined_features[3], combined_lossy_features[3])

            # Encode the combined image and the style image and use styleLoss() to get the style loss of the image
            loss_s = 0.0
            for i in range(len(combined_lossy_features)):
                loss_s += self.style_loss(combined_lossy_features[i], style_features[i])

            return loss_c, loss_s
        else:  # inference
            #This is for if the function is not training since we will not need the loss
            #This will just give the final image
            #   your code here ...
            content_features = self.encode(content)
            style_features = self.encode(style)
            ...
            # Combine the encoded images using AdaIn, this will give you the combined encoded image
            # Could be style_features[-1]
            feat = self.adain(content_features, style_features)
