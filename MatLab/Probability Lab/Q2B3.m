...
axis_a.XLim = [0,100];
axis_a.YLabel.String = 'f_1(RV3)';

% Estimate the mean 
mean = sum(RV3)/length(RV3);

% Estimate the variance
variance = 0;
for i =1:length(RV3)
    variance = variance +(RV3(i) - mean)^2;
end
variance = variance / length(RV3);
